class OrderWalkthrough
  def self.up_to(state)
    ...
    store = Store.default || FactoryBot.create(:store, default: true)

    # A payment method must exist for an order to proceed through the Address state
    unless PaymentMethod.exists?
    ...
    order = Order.create!(email: 'spree@example.com')
    add_line_item!(order)
    order.next!
  ...
  def self.address(order)
    order.bill_address = FactoryBot.create(:address, country_id: Zone.global.members.first.zoneable.id)
    order.ship_address = FactoryBot.create(:address, country_id: Zone.global.members.first.zoneable.id)
    order.next!
  end

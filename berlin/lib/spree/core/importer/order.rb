      ...
      class Order
        ...
        def self.create_shipments_from_params(shipments_hash, order)
          ...
          shipments_hash.each do |s|
            ...
            shipping_method =ShippingMethod.find_by(name: s[:shipping_method]) ||
             ShippingMethod.find_by!(admin_name: s[:shipping_method])
        ...
        def self.create_payments_from_params(payments_hash, order)
          ...
          payments_hash.each do |p|
            ...
            payment.payment_method = PaymentMethod.find_by!(name: p[:payment_method])
        ...
        def self.ensure_variant_id_from_params(hash)
          ...
          unless hash[:variant_id].present?
            hash[:variant_id] = Variant.active.find_by!(sku: sku).id
          end
          hash
        ...
        def self.ensure_country_id_from_params(address)
          ...
          begin
            ...
            address[:country_id] = Country.where(search).first!.id
          rescue Exception => e

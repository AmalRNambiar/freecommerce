module Spree
  module Core
    ...
    module ProductFilters
      ...
      #
      Product.add_search_scope :price_range_any do |*opts|
        conds = opts.map { |o| Spree::Core::ProductFilters.price_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Product.joins(master: :default_price).where(scope)
      end
      ...
      Product.add_search_scope :brand_any do |*opts|
      ...
      Product.add_search_scope :selective_brand_any do |*opts|
        Product.brand_any(*opts)
      end
      ...
      def self.selective_brand_filter(taxon = nil)
        ...
        scope = Spree::ProductProperty.where(property: brand_property).
                ...
                where("#{Taxon.table_name}.id" => [taxon] + taxon.descendants)

class RecreateSpreeReturnAuthorizations < ActiveRecord::Migration[4.2]
  ...
  def down
    ...
    Adjustment.where(source_type: 'Spree::LegacyReturnAuthorization').update_all(source_type: 'Spree::ReturnAuthorization')

    if data_source_exists?(:legacy_return_authorizations)
      rename_table :legacy_return_authorizations, :return_authorizations
      rename_column :inventory_units, :legacy_return_authorization_id, :return_authorization_id
    else
      create_table :return_authorizations do |t|
        t.string   "number"
        t.string   "state"
      ...
      add_column :inventory_units, :return_authorization_id, :integer, after: :shipment_id

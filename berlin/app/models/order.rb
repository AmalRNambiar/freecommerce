require_dependency 'order/checkout'
require_dependency 'order/currency_updater'
require_dependency 'order/payments'
require_dependency 'order/store_credit'
...
class Order < Base
  ...
  with_options dependent: :destroy do
    has_many :state_changes, as: :stateful, class_name: 'StateChange'
    has_many :line_items, -> { order(:created_at) }, inverse_of: :order, class_name: 'LineItem'
    has_many :payments, class_name: 'Payment'
    has_many :return_authorizations, inverse_of: :order, class_name: 'ReturnAuthorization'
    has_many :adjustments, -> { order(:created_at) }, as: :adjustable, class_name: 'Adjustment'
  end
  ...
  def find_line_item_by_variant(variant, options = {})
    line_items.detect do |line_item|
      line_item.variant_id == variant.id &&
        Dependencies.cart_compare_line_items_service.constantize.new.call(order: self, line_item: line_item, options: options).value
    end
  end
  ...
  def validate_payments_attributes(attributes)
    ...
    payment_methods = PaymentMethod.where(id: available_payment_methods.map(&:id))

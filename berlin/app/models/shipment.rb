...
class Shipment < Base
  ...
  scope :reverse_chronological, -> { order(Arel.sql('coalesce(shipments.shipped_at, shipments.created_at) desc'), id: :desc) }
  ...
  def free?
    ...
    adjustments.promotion.any? { |p| p.source.type == 'Promotion::Actions::FreeShipping' }
  end

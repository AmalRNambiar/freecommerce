class Reimbursement < Base
  # include Core::NumberGenerator.new(prefix: 'RI', length: 9)
  ...
  with_options inverse_of: :reimbursement do
    has_many :refunds
    has_many :credits, class_name: 'Reimbursement::Credit'
  ...
  def display_total
    Money.new(total, currency: order.currency)
  end
  ...
  def send_reimbursement_email
    ReimbursementMailer.reimbursement_email(id).deliver_later
  end
